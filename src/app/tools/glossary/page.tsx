...
/** EN labels for the 8 glossary categories. */
const CATEGORY_LABEL_EN: Record<GlossaryCategory, string> = {
  documentos:   "Documents",
  contenedores: "Containers",
  transporte:   "Transport",
  aduanas:      "Customs",
  tarifas:      "Fees & charges",
...
const CATEGORIES: { id: "all" | GlossaryCategory; label: string }[] = [
  { id: "all",          label: "All" },
  { id: "documentos",   label: CATEGORY_LABEL_EN.documentos },
  { id: "contenedores", label: CATEGORY_LABEL_EN.contenedores },
  { id: "transporte",   label: CATEGORY_LABEL_EN.transporte },
  { id: "aduanas",      label: CATEGORY_LABEL_EN.aduanas },
  { id: "tarifas",      label: CATEGORY_LABEL_EN.tarifas },
  { id: "operaciones",  label: CATEGORY_LABEL_EN.operaciones },
  { id: "latam",        label: CATEGORY_LABEL_EN.latam },
];
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-6 text-base md:text-lg text-gv-muted max-w-3xl leading-relaxed">
              Dictionary of international trade terms, acronyms, and concepts with a
              LATAM focus.{" "}
          ...
          <div className="bg-white rounded-card border border-gv-border shadow-card p-5">
            <div className="relative mb-4">
              ...
              <input
                ...
                placeholder="Search by term, acronym, or concept…"
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-card border border-gv-border shadow-card p-12 text-center">
            ...
            <p className="mt-4 text-gv-muted">
              No matches for your search. Try fewer letters or a different category.
            </p>
          </div>
        ) : (
                ...
                {terms.map((t) => (
                  <TermCard
                    ...
                    onSelectRelated={(slug) => {
                      ...
                      if (target) {
                        const head = (target.acronym ?? target.term)
                          .charAt(0)
                          .toUpperCase();
                        document
                          .getElementById(`letter-${head}`)
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20">
        <div className="bg-gv-blue text-white rounded-card p-8 md:p-10">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              <h3 className="text-2xl md:text-3xl font-bold">Missing a term?</h3>
              <p className="mt-3 text-white/80 max-w-2xl leading-relaxed">
                If you didn&apos;t find what you were looking for, drop us a note.
                We grow the glossary with terms the LATAM community actually uses.
              </p>
            </div>
            <BrandButton
    ...
    >
      <header className="mb-3">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          <span
            className={`text-[10px] font-bold tracking-widest uppercase px-2 py-0.5 rounded ${CATEGORY_COLORS[term.category]}`}
          >
            {CATEGORY_LABEL_EN[term.category]}
          </span>
          {term.tags?.map((tag) => (
            <span
              key={tag}
