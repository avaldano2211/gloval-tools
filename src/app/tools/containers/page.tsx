...
export default function ContainersPage() {
  ...
  const groups: { label: string; ids: string[] }[] = [
    { label: "Standard (DC)", ids: ["20gp", "40gp", "40hc", "45hc"] },
    { label: "Reefer", ids: ["20rf", "40rfhc"] },
    ...
    <div className="bg-gv-bg-soft min-h-screen pt-24">
      ...
      <header className="max-w-7xl mx-auto px-6 pt-6 pb-12">
        ...
        <p className="mt-6 text-base md:text-lg text-gv-muted max-w-3xl leading-relaxed">
          Dimensions, weight, and capacity for each ISO container type. Select a
          type to see its full technical sheet.
        </p>
      </header>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-card border border-gv-border shadow-card p-8">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              <SpecBlock title="Internal dimensions" rows={[
                ["Length", `${spec.internal.length_m.toFixed(3)} m / ${(spec.internal.length_m * 3.281).toFixed(2)} ft`],
                ["Width",  `${spec.internal.width_m.toFixed(3)} m / ${(spec.internal.width_m * 3.281).toFixed(2)} ft`],
                ["Height", `${spec.internal.height_m.toFixed(3)} m / ${(spec.internal.height_m * 3.281).toFixed(2)} ft`],
              ]} />
              <SpecBlock title="External dimensions" rows={[
                ["Length", `${spec.external.length_m.toFixed(3)} m`],
                ["Width",  `${spec.external.width_m.toFixed(3)} m`],
                ["Height", `${spec.external.height_m.toFixed(3)} m`],
              ]} />
              {(spec.door.width_m > 0) && (
                <SpecBlock title="Door dimensions" rows={[
                  ["Width",  `${spec.door.width_m.toFixed(3)} m`],
                  ["Height", `${spec.door.height_m.toFixed(3)} m`],
                ]} />
              )}
              <SpecBlock title="Weights" rows={[
                ["Tare",        `${spec.weights.tare_kg.toLocaleString()} kg`],
                ["Payload",     `${spec.weights.payload_kg.toLocaleString()} kg`],
                ["Max gross",   `${spec.weights.max_gross_kg.toLocaleString()} kg`],
              ]} />
              <SpecBlock title="Capacity" rows={[
                ["Volume", `${spec.capacity.cbm.toFixed(1)} m³ / ${spec.capacity.cuft.toLocaleString()} ft³`],
                ["US pallets 48×40",  `${spec.pallets.us_48x40} units`],
                ["EUR pallets 120×80", `${spec.pallets.eur_120x80} units`],
              ]} />
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-gv-border flex flex-col sm:flex-row gap-3">
              <BrandButton variant="filled" href="https://www.glovalshipping.com/request-a-quote">
                Quote this container
              </BrandButton>
              <BrandButton variant="secondary" href={`/tools/calculator?from=${spec.id}`}>
                Calculate volume
              </BrandButton>
            </div>
          </div>
      ...
      <section className="bg-white">
        <div className="max-w-7xl mx-auto px-6 py-20">
          ...
          <H2 className="!text-3xl md:!text-4xl">Road weight limits by country</H2>
          <p className="mt-6 text-base text-gv-muted max-w-3xl">
            Max gross road weight varies by country — important for FCL legs to
            or from LATAM ports with long inland transport.
          </p>
            ...
            {LATAM_ROAD_LIMITS.map((c) => (
              ...
              >
                ...
                <div className="mt-3 text-3xl font-extrabold text-gv-blue">
                  ...
                  <span className="text-sm font-medium text-gv-muted ml-1">max tons</span>
                </div>
      ...
      <section className="bg-gv-bg-soft">
        <div className="max-w-7xl mx-auto px-6 py-20">
          ...
          <H2 className="!text-3xl md:!text-4xl">Does it accept 45&apos; HC?</H2>
          <p className="mt-6 text-base text-gv-muted max-w-3xl">
            Not every LATAM port handles 45&apos; High Cube containers. Check
            before you quote.
          </p>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gv-blue text-white">
                <tr>
                  ...
                  <th className="text-center px-6 py-4 font-semibold">Accepts 45&apos; HC</th>
          ...
          <p className="mt-6 text-xs text-gv-muted italic">
            Reference information. Verify with your Gloval agent for your
            specific shipment — operational conditions may change.
          </p>
        </div>
      </section>
    </div>
