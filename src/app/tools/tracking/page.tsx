...
const TABS: { id: SearchType; label: string; placeholder: string; example: string }[] = [
  { id: "container", label: "Container",      placeholder: "ABCD1234567",                    example: "e.g. MAEU1234567" },
  { id: "bl",        label: "Bill of Lading", placeholder: "BL number",                      example: "e.g. MAEU987654321" },
  { id: "vessel",    label: "Vessel",         placeholder: "Vessel name or IMO",             example: "e.g. MAERSK SEMARANG" },
];
...
/** EN labels for event types (lib has ES). */
const EVENT_LABEL_EN: Record<EventType, string> = {
  gate_in:      "Gate-in",
...
/** EN labels for status (lib has ES). */
const STATUS_LABEL_EN: Record<TrackingStatus, string> = {
...
/** EN translations for mock event descriptions present in lib/tracking.ts. */
const EVENT_DESC_EN: Record<string, string> = {
  "Gate-in en terminal de origen":   "Gate-in at origin terminal",
  "Cargado a bordo":                 "Loaded on board",
  "Zarpe del puerto de origen":      "Departed origin port",
  "Zarpe":                           "Departed",
  "Transbordo programado":           "Scheduled transshipment",
  "Transbordo en hub":               "Transshipment at hub",
  "Transbordo":                      "Transshipment",
  "Arribo a puerto destino":         "Arrived at destination port",
  "Arribo a Callao":                 "Arrived at Callao",
  "Descarga del buque":              "Discharged from vessel",
  "Descargado del buque":            "Discharged from vessel",
  "Salida de terminal":              "Out of terminal",
  "Entregado a consignatario":       "Delivered to consignee",
  "Gate-in":                         "Gate-in",
};
    ...
    <div className="bg-gv-bg-soft min-h-screen pt-24">
      ...
      <header className="max-w-7xl mx-auto px-6 pt-6 pb-12">
        ...
        <p className="mt-6 text-base md:text-lg text-gv-muted max-w-3xl leading-relaxed">
          Track containers, Bills of Lading, and vessels with live events, ETA,
        ...
        <div className="bg-white rounded-card border border-gv-border shadow-card p-6 md:p-8">
          ...
          <form onSubmit={handleSearch}>
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <BrandButton type="submit" variant="filled" className="shrink-0 !px-8" disabled={loading}>
                {loading ? (
                  <>
        ...
        <div className="mt-6 bg-gv-orange/10 border border-gv-orange/30 rounded-card p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-gv-blue leading-relaxed">
              <strong className="font-bold">Demo version.</strong> This tool uses simulated data to show the experience. The production version consumes a real tracking API (searates.com / shipsgo.com / similar) covering Maersk, MSC, CMA-CGM, Evergreen, Hapag-Lloyd, ONE, COSCO, HMM, ZIM and other major carriers.
            </div>
          </div>
        </div>
        ...
        {notFound && !loading && (
          <div className="mt-6 bg-white border border-red-200 rounded-card p-8 text-center">
            ...
            <h3 className="mt-4 text-lg font-bold text-gv-blue">
              We didn&apos;t find &ldquo;{notFound}&rdquo;
            </h3>
            <p className="mt-2 text-sm text-gv-muted max-w-md mx-auto">
              In the demo version we only respond to the example numbers above.
              In production, the API hits the carrier directly.
            </p>
          </div>
        )}
...
function TrackingResult({ data }: { data: Tracking }) {
  ...
  const statusLabelEn = STATUS_LABEL_EN[data.status];
  return (
    <div className="mt-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold tracking-[0.2em] uppercase text-gv-orange">
                Gloval Shipping shipment
              </div>
              <div className="text-lg font-bold mt-1">
                This BL belongs to one of our operations
              </div>
            </div>
          </div>
          <BrandButton
            href="https://www.glovalshipping.com/"
      ...
      <div className="bg-white rounded-card border border-gv-border shadow-card p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-10">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-xs font-bold tracking-[0.15em] uppercase text-gv-muted">
              {data.type === "container" ? "Container" : data.type === "bl" ? "Bill of Lading" : "Vessel"}
            </div>
          ...
          <div className="flex flex-col items-start md:items-end shrink-0">
            ...
            <div className="mt-3 text-right">
              ...
              <div className="text-lg font-bold text-gv-blue mt-1">
                {new Date(data.eta).toLocaleDateString("en", { day: "numeric", month: "short", year: "numeric" })}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 md:gap-6 items-center">
          <PortBlock label="Origin" port={data.origin.port} locode={data.origin.locode} />
          <Ship className="size-7 text-gv-orange justify-self-center hidden md:block" strokeWidth={1.5} />
          <PortBlock label="Destination" port={data.destination.port} locode={data.destination.locode} alignRight />
        </div>
      </div>
        ...
        <div className="bg-white rounded-card border border-gv-border shadow-card p-6 md:p-8">
          ...
          <ol className="relative">
            ...
            {data.events.slice().reverse().map((e, i) => {
              ...
              const isLatest = i === 0;
              const descEn = EVENT_DESC_EN[e.description] ?? e.description;
              return (
      ...
      <div className="bg-white rounded-card border border-gv-border shadow-card p-6 md:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
        <div>
          ...
          <h4 className="text-lg font-bold text-gv-blue mt-1">
            Quote with Gloval and track from your client portal.
          </h4>
        </div>
      ...
      <p className="text-xs text-gv-muted italic">
        In production, data comes from carriers via API and may lag by 4–24 hours.
        For real-time data, ask your Gloval agent.
      </p>
    </div>
  );
}
